...
func TestOrgInvitation(t *testing.T) {
	type restartAgolaFn func(orgInvitiationEnabled bool)

	type testOrgInvitationConfig struct {
		sc             *setupContext
		restartAgola   restartAgolaFn
		tokenUser01    string
		tokenUser02    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			restartAgola := func(orgInvitiationEnabled bool) {
				if orgInvitiationEnabled {
					sc.config.Gateway.OrganizationMemberAddingMode = config.OrganizationMemberAddingModeInvitation
				} else {
					sc.config.Gateway.OrganizationMemberAddingMode = config.OrganizationMemberAddingModeDirect
				}
